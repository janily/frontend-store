...
const js_snap = document.querySelector('.thnos-snap')

const js_hand = document.querySelector('.hand')

const js_snaping = document.querySelector('.snap')


js_hand.addEventListener('click', function () {

  js_hand.style.display = "none";


  js_snaping.classList.remove("hide")

  /** === Below is just to bind the module and the DOM == */
  document.querySelectorAll(".thnos").forEach($elm => {
  ...
  setTimeout(() => {
    ...
    js_hand.style.display = "block";


    js_snaping.classList.add("hide")
  }, 2500)
})


// Time Effect
const gauntlet = document.querySelector(".hand_time");
const time = document.querySelector(".time");
const thnos_reverse = document.querySelector(".thnos")

gauntlet.addEventListener("click", timeEffect);

function timeEffect() {
  gauntlet.classList.add("hide");
  time.classList.remove("hide");
  thnos_reverse.style.visibility = "visible";
  setTimeout(() => {
    gauntlet.classList.remove("hide");
